...
import { Network, BalancerSDK, SwapType, Swaps } from "@balancer-labs/sdk";
...
@Injectable()
export class BalancerWeb3Service {
  constructor(private readonly priceService: PriceService) {}
  config = {
    network: Network.ARBITRUM,
    rpcUrl: process.env.ALCHEMY_URL,
  };
  // swaps = new Swaps(config);
  abi = [
    {
      inputs: [
        {
  ...
  // provider = new AlchemyProvider("arbitrum", process.env.ALCHEMY_API_KEY);
  ...
  async queryBatchSwap(steps: BatchSwapStep[], assets: string[]) {
    const swaps = new Swaps(this.config);
    const result = await swaps.queryBatchSwap({
    ...
    // };
    // const result = await this.vault.queryBatchSwap(0, steps, assets, funds);
    const prices = [];
    const decimals = [];
    const dollarAmounts = [];
    for (let i = 0; i < assets.length; i++) {
      const element = assets[i];
      prices.push(await this.priceService.getPrice(element));
      decimals.push(await this.priceService.getDecimals(element));
      const delta = parseFloat(
        ethers.utils.formatUnits(result[i], decimals[i]),
      );
      dollarAmounts.push(delta * prices[i]);
    }
    console.log("dollarAmounts ", dollarAmounts);
    console.log("Bot Result USD", -sum(dollarAmounts));
    // console.log(prices);
    // console.log(decimals);
    // const prices = await Promise.all(
    //   assets.map(async (a) => {
    //     await this.priceService.getPrice(a);
    //   }),
    // );

    // result.forEach((r) => {
    //   console.log(ethers.utils.formatUnits(r, 6));
    // });
    console.log(result);
    return result;
  }
